    ...
    def __init__(self, model_dir):
        ...
        if not model_dir:
            raise ValueError(
                ...
                "the deepdoc model files (e.g. tsr.onnx)."
            )
        # If model file doesn't exist, try downloading from HuggingFace
        model_file = os.path.join(model_dir, "tsr.onnx")
    ...
    def blockType(b):
        ...
        for p, n in patt:
            if re.search(p, b["text"].strip()):
                return n
        tks = [t for t in b["text"].split() if len(t) > 1]
        if len(tks) > 3:
            if len(tks) < 12:
                return "Tx"
            else:
