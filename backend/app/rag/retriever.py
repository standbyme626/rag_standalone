...
class MedicalRetriever:
    ...
    def __init__(self):
        ...
        siliconflow_enabled = bool(getattr(settings, "SILICONFLOW_ENABLED", True))
        use_siliconflow = siliconflow_enabled and bool(siliconflow_key)

        if use_siliconflow or os.path.exists(rerank_path):
    ...
    async def analyze_intent(self, query: str) -> Dict[str, Any]:
        ...
        """
        try:
            prompt = intent_classification(query)
    ...
    async def _rewrite_query_async(self, query: str) -> str:
        ...
        if bool(getattr(settings, "RAG_PURE_RETRIEVAL_MODE", False)) and bool(
            ...
            return query
        try:
            prompt = query_rewrite(query)
            results = await local_slm.generate_batch_async(
                [prompt], system_prompt=system_rewrite()
            )
    ...
    async def summarize_batch(
        ...
        prompts = []
        for t in texts:
            safety_instruction = safety_disclaimer() if low_confidence else ""

            p = summarization(
                query=query, document=t, safety_instruction=safety_instruction
            )
            prompts.append(p)
