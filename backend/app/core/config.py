...
class Settings(BaseSettings):
    ...
    SILICONFLOW_ENABLED: bool = (
        os.getenv("SILICONFLOW_ENABLED", "true").lower() == "true"
    )
    ...
    # === PLUGIN CONFIG ===
    PLUGINS_ENABLED: str = os.getenv("PLUGINS_ENABLED", "medical")
    RETRIEVAL_PLUGIN: str = os.getenv("RETRIEVAL_PLUGIN", "medical")

    # === INGESTION PIPELINE CONFIG ===
    INGESTION_CHUNK_SIZE: int = int(os.getenv("INGESTION_CHUNK_SIZE", "512"))
    INGESTION_CHUNK_OVERLAP: int = int(os.getenv("INGESTION_CHUNK_OVERLAP", "64"))
    INGESTION_CHUNK_STRATEGY: str = os.getenv("INGESTION_CHUNK_STRATEGY", "recursive")
    INGESTION_PII_REDACTION: bool = os.getenv("INGESTION_PII_REDACTION", "true").lower() == "true"
    INGESTION_DEDUP: bool = os.getenv("INGESTION_DEDUP", "true").lower() == "true"
    INGESTION_COLLECTION_NAME: str = os.getenv(
        "INGESTION_COLLECTION_NAME", "huatuo_knowledge"
    )
